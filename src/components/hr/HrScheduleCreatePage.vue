...
const presetIdSet = new Set(presetStudentIds)
const presetApplied = ref(false)
// ダッシュボードから来たときは、選ばれている候補者だけを一覧に出す。
// 他の人も足したくなったら「全員から選ぶ」で切り替えられる
const presetOnly = ref(presetStudentIds.length > 0)
...
const filteredStudents = computed(() => students.value.filter((student) => {
  ...
  return (!presetOnly.value || presetIdSet.has(student.id))
  ...
  <div class="page">
    ...
    <form class="layout" @submit.prevent="submit">
      <section class="card candidates">
        ...
        <div class="filters"><label><HrIcon name="search" :size="16"/><input v-model="search" placeholder="名前・メールで検索"></label><select v-model="selectionFilter"><option value="all">すべての選考段階</option><option v-for="(label,key) in stages" :key="key" :value="key">{{ label }}</option></select><button v-if="presetStudentIds.length" type="button" class="scope" :class="{'scope--on':presetOnly}" @click="presetOnly=!presetOnly">{{ presetOnly ? `選ばれた${presetStudentIds.length}名だけ表示中／全員から選ぶ` : `全員を表示中／選ばれた${presetStudentIds.length}名にしぼる` }}</button></div>
...
<style scoped>
.page,.page *{box-sizing:border-box}.page{height:100%;overflow:auto;padding:28px clamp(20px,3vw,45px);background:#f7f9fc;color:#273348}.page-head{display:flex;align-items:end;justify-content:space-between;margin-bottom:20px}.page-head>div>span{color:#1769ff;font-size:12px;font-weight:800;letter-spacing:.15em}.page-head h1{margin:5px 0 0;font-size:24px}.page-head p{margin:6px 0 0;color:#78849a;font-size:13px}.page-head>strong{color:#1769ff;font-size:25px}.page-head>strong small{margin-left:6px;color:#758197;font-size:12px}.layout{display:grid;grid-template-columns:minmax(420px,1.08fr) minmax(410px,.92fr);gap:14px}.card{overflow:hidden;border:1px solid #e0e6ef;border-radius:13px;background:#fff}.card>header{display:flex;min-height:62px;align-items:center;justify-content:space-between;border-bottom:1px solid #e8edf3;padding:0 16px}.card>header>div{display:flex;align-items:center;gap:10px}.card>header b{display:grid;width:27px;height:27px;place-items:center;border-radius:8px;background:#eaf1ff;color:#1769ff;font-size:13px}.card header span{display:flex;flex-direction:column}.card header strong{font-size:13px}.card header small{margin-top:3px;color:#8b95a6;font-size:12px}.card header button{border:0;background:none;color:#1769ff;font-size:12px;font-weight:700;cursor:pointer}.candidates{grid-row:span 2}.filters{display:grid;grid-template-columns:1fr 165px;gap:8px;padding:12px;border-bottom:1px solid #edf0f4}.filters label{display:flex;height:36px;align-items:center;gap:7px;border:1px solid #dce3ed;border-radius:8px;padding:0 10px}.filters input{min-width:0;flex:1;border:0;outline:0;font-size:12px}.filters select{border:1px solid #dce3ed;border-radius:8px;background:#fff;padding:0 8px;font-size:12px}
.filters .scope{grid-column:1/-1;height:32px;border:1px solid #dce3ed;border-radius:8px;background:#fff;color:#55637c;font:inherit;font-size:12px;font-weight:700;cursor:pointer}
.filters .scope:hover{border-color:#b7c9e9;background:#f7faff}
.filters .scope--on{border-color:#bcd4ff;background:#f2f7ff;color:#1156c9}.list{height:493px;overflow:auto;padding:7px 10px}.list>label{display:flex;min-height:57px;align-items:center;gap:9px;border:1px solid transparent;border-radius:9px;padding:7px 9px;cursor:pointer}.list>label.selected{border-color:#acc6fb;background:#f2f6ff}.list input,.switch input{position:absolute;opacity:0}.list>label>i{display:grid;width:33px;height:33px;place-items:center;border-radius:50%;background:#e7f7f1;color:#16845f;font-size:13px;font-style:normal;font-weight:800}.list label>span{display:flex;min-width:0;flex:1;flex-direction:column}.list label span strong{font-size:12px}.list label span small{margin-top:3px;color:#8994a6;font-size:12px}.list label>em{border-radius:8px;padding:4px 7px;background:#edf3ff;color:#1769ff;font-size:12px;font-style:normal}.list label>b{display:grid;width:18px;height:18px;place-items:center;border:1px solid #cbd3df;border-radius:5px;color:transparent}.list label.selected>b{border-color:#1769ff;background:#1769ff;color:#fff}.empty{padding:55px;text-align:center;color:#8994a6;font-size:12px}.settings{display:flex;flex-direction:column;gap:14px}.body{padding:14px}.switch{display:grid;grid-template-columns:1fr 1fr;gap:6px}.switch span{display:flex;height:35px;align-items:center;justify-content:center;border:1px solid #dce3ed;border-radius:8px;font-size:12px;cursor:pointer}.switch input:checked+span{border-color:#1769ff;background:#edf3ff;color:#1769ff}.note{color:#768398;font-size:12px}.people{display:flex;max-height:65px;flex-wrap:wrap;gap:6px;overflow:auto;margin-top:10px}.people label{border-radius:7px;padding:5px 7px;background:#f4f6f9;font-size:12px}.field{display:flex;flex-direction:column;margin-top:12px}.field>span{margin-bottom:6px;font-size:12px;font-weight:700}.field>span i{color:#1769ff;font-size:12px}.field>div{position:relative}.field input,.field textarea{width:100%;border:1px solid #dce3ed;border-radius:7px;padding:0 9px;outline:0;font:inherit;font-size:12px}.field input{height:35px}.field textarea{padding:8px;resize:vertical}.field>div>em{position:absolute;right:9px;top:50%;transform:translateY(-50%);color:#7c8799;font-size:12px;font-style:normal}.grid{display:grid;grid-template-columns:1.2fr .8fr;gap:0 10px}.wide{grid-column:1/-1}.dates{display:grid!important;grid-template-columns:1fr 15px 1fr;align-items:center;gap:4px}.dates>em{position:static!important;transform:none!important;text-align:center}.deadline{grid-template-columns:1.3fr .7fr}.grid aside{display:flex;align-items:center;gap:8px;margin-top:12px;border:1px solid #cdddfb;border-radius:8px;padding:8px;background:#f2f6ff;color:#1769ff}.grid aside span{display:flex;flex-direction:column}.grid aside strong{font-size:12px}.grid aside small{font-size:12px;color:#687b99}.field>small{text-align:right;color:#929cad;font-size:12px}.layout>footer{grid-column:1/-1;display:flex;align-items:center;justify-content:flex-end;gap:18px;border:1px solid #e0e6ef;border-radius:12px;padding:12px 15px;background:#fff}.layout>footer p{margin:0;color:#758196;font-size:12px}.layout>footer p.error{color:#c74242}.layout>footer button{display:flex;min-width:210px;height:40px;align-items:center;justify-content:center;gap:7px;border:0;border-radius:8px;background:#1769ff;color:#fff;font-size:12px;font-weight:800}.layout>footer button:disabled{background:#c7d2e5}@media(max-width:1000px){.layout{grid-template-columns:1fr}.list{height:330px}.layout>footer{grid-column:1}}@media(max-width:600px){.page{padding:80px 13px 25px}.filters,.grid{grid-template-columns:1fr}.wide{grid-column:1}.layout>footer{align-items:stretch;flex-direction:column}.layout>footer button{width:100%}}
</style>
