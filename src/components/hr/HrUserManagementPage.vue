...
const createUser = async (user) => {
  ...
  try {
    ...
    users.value.unshift({
      ...
      organization: "",
      status: "active",
      createdAt: new Intl.DateTimeFormat("ja-JP").format(new Date(result.createdAt)),
    })
...
<style scoped>
.users-page { height: 100%; overflow-y: auto; padding: 34px clamp(24px, 4.5vw, 68px) 50px; background: #f7f9fc; color: #263248; }.page-header { display: flex; align-items: flex-end; justify-content: space-between; gap: 24px; }.eyebrow { display: block; margin-bottom: 7px; color: #1769ff; font-size: 8px; font-weight: 800; letter-spacing: .16em; }.page-header h1 { margin: 0; color: #172033; font-size: 25px; letter-spacing: -.03em; }.page-header p { margin: 7px 0 0; color: #748096; font-size: 11px; }.add-button { display: flex; height: 42px; align-items: center; gap: 8px; border: 0; border-radius: 9px; padding: 0 17px; background: #1769ff; box-shadow: 0 7px 16px rgb(23 105 255 / 23%); color: #fff; font-size: 11px; font-weight: 750; cursor: pointer; }.add-button:hover { background: #0758ed; }
.permission-note { display: flex; align-items: center; gap: 11px; margin-top: 24px; border: 1px solid #cdddfb; border-radius: 10px; padding: 12px 15px; background: #f1f6ff; }.permission-note > span { display: grid; width: 34px; height: 34px; flex: 0 0 auto; place-items: center; border-radius: 9px; background: #fff; color: #1769ff; }.permission-note div { display: flex; min-width: 0; align-items: baseline; gap: 12px; }.permission-note strong { color: #24467d; font-size: 10px; }.permission-note p { margin: 0; color: #657692; font-size: 9px; }
.summary-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin: 18px 0; }.summary-grid article { display: flex; align-items: center; gap: 12px; border: 1px solid #e2e8f0; border-radius: 11px; padding: 15px; background: #fff; }.summary-icon { display: grid; width: 38px; height: 38px; place-items: center; border-radius: 10px; }.summary-icon--all { background: #edf3ff; color: #1769ff; }.summary-icon--student { background: #ecf9f4; color: #14855d; }.summary-icon--interviewer { background: #fff4e8; color: #c16b16; }.summary-icon--hr { background: #f4edff; color: #7445c5; }.summary-grid article div { display: flex; flex-direction: column; }.summary-grid small { color: #7a869a; font-size: 8px; }.summary-grid strong { margin-top: 3px; color: #202b3e; font-size: 20px; }
.user-panel { overflow: hidden; border: 1px solid #e0e6ef; border-radius: 12px; background: #fff; box-shadow: 0 4px 14px rgb(32 48 78 / 4%); }.panel-tools { display: flex; min-height: 66px; align-items: center; justify-content: space-between; gap: 18px; padding: 0 18px; border-bottom: 1px solid #e8edf3; }.tabs { display: flex; height: 66px; align-items: stretch; gap: 4px; }.tabs button { position: relative; display: flex; align-items: center; gap: 6px; border: 0; padding: 0 12px; background: transparent; color: #667289; font-size: 10px; font-weight: 700; cursor: pointer; }.tabs button::after { position: absolute; right: 10px; bottom: 0; left: 10px; height: 2px; border-radius: 4px; background: transparent; content: ""; }.tabs button.active { color: #1769ff; }.tabs button.active::after { background: #1769ff; }.tabs button span { min-width: 18px; border-radius: 8px; padding: 2px 5px; background: #f0f3f7; color: #758095; font-size: 8px; }.tabs button.active span { background: #e8f0ff; color: #1769ff; }
.search { display: flex; width: min(220px, 32%); height: 36px; align-items: center; gap: 8px; border: 1px solid #dce3ed; border-radius: 8px; padding: 0 10px; color: #8994a7; }.search:focus-within { border-color: #1769ff; box-shadow: 0 0 0 3px rgb(23 105 255 / 8%); }.search input { min-width: 0; flex: 1; border: 0; outline: 0; color: #344054; font: inherit; font-size: 9px; }
.table-wrap { overflow-x: auto; }table { width: 100%; border-collapse: collapse; }th { height: 40px; background: #fafbfd; color: #8993a5; font-size: 8px; font-weight: 700; text-align: left; white-space: nowrap; }th,td { padding: 0 16px; }td { height: 66px; border-top: 1px solid #edf0f4; color: #5e6a7f; font-size: 9px; white-space: nowrap; }.user-cell { display: flex; align-items: center; gap: 10px; }.user-cell > div { display: flex; flex-direction: column; }.user-cell strong { color: #2b374b; font-size: 10px; }.user-cell small { margin-top: 4px; color: #8590a2; font-size: 8px; }.avatar { display: grid; width: 34px; height: 34px; place-items: center; border-radius: 50%; font-size: 10px; font-weight: 800; }.avatar--student { background: #dcf4e9; color: #157653; }.avatar--interviewer { background: #ffead3; color: #a85c11; }.avatar--hr { background: #eadffd; color: #6940ad; }
.role-badge { display: inline-flex; align-items: center; gap: 5px; border-radius: 7px; padding: 5px 8px; font-size: 8px; font-weight: 750; }.role-badge--student { background: #edf9f4; color: #157653; }.role-badge--interviewer { background: #fff5e9; color: #a85c11; }.role-badge--hr { background: #f4effd; color: #6940ad; }.status { display: inline-flex; align-items: center; gap: 6px; }.status i { width: 6px; height: 6px; border-radius: 50%; }.status--active i { background: #20aa70; box-shadow: 0 0 0 3px #e5f7ef; }.status--invited { color: #aa6b1c; }.status--invited i { background: #efa83f; box-shadow: 0 0 0 3px #fff3df; }.more-button { display: grid; width: 30px; height: 30px; place-items: center; border: 0; border-radius: 7px; background: transparent; color: #788499; cursor: pointer; }.more-button:hover { background: #f1f4f8; }
.empty { display: flex; min-height: 230px; flex-direction: column; align-items: center; justify-content: center; }.empty > span { display: grid; width: 42px; height: 42px; place-items: center; border-radius: 11px; background: #f1f4f8; color: #7e8a9e; }.empty strong { margin-top: 12px; font-size: 11px; }.empty p { margin: 4px 0 0; color: #8994a7; font-size: 9px; }.panel-footer { padding: 12px 17px; border-top: 1px solid #e8edf3; color: #8791a3; font-size: 8px; }
.toast { position: fixed; z-index: 70; top: 24px; left: 50%; display: flex; align-items: center; gap: 9px; transform: translateX(-50%); border: 1px solid #dce5f2; border-radius: 10px; padding: 11px 16px; background: #fff; box-shadow: 0 10px 30px rgb(24 37 65 / 16%); color: #293750; font-size: 10px; font-weight: 700; }.toast span { display: grid; width: 21px; height: 21px; place-items: center; border-radius: 50%; background: #e6f7ee; color: #18955a; }.toast-enter-active,.toast-leave-active { transition: opacity .2s, transform .2s; }.toast-enter-from,.toast-leave-to { opacity: 0; transform: translate(-50%, -7px); }
