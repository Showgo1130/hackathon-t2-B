...
- 実装は Claude Code / GitHub Copilot、画面案は ChatGPT の画像生成を使っています
...
いちばん時間を使ったのは実装の前です。顧客の課題を解決するには何が必要なのかを先に洗い切ったので、手を動かし始めた時点で、作るものはある程度決まっていました。

そのうえで、こういう順番で進めました。

1. 動くモックを作って、チーム全員でイメージを合わせる
2. 具体的な画面案を ChatGPT の画像生成で作り、それを見ながらもう一度すり合わせる
3. 個人作業に分かれて実装する
4. 終盤は画面を共有しながら、UI も含めてその場で決めていく

UI を直すときも同じで、作る前に画像を出して「これでいいか」を確認してから着手しました。できあがってから作り直す、という時間がほとんど発生していません。

完成したあとは逆に、要らないものを削りました。顧客が実際に何をするのかという視点で画面を見直して、使われない機能や表示を落としています。

やってみて分かったのは、AI が実装を返す速度が速いぶん、詰まるのは「どうするかを決めるところ」になるということです。決めるための材料を先に用意しておけば、そこで止まりません。
...
- 実装だけでなく、画面案の作成にも ChatGPT の画像生成を使っています（「進め方」を参照）

チームで行っていたのは、何を作るか、それが誰のどんな課題を解決するか、その解決策で本当にその課題を解決できるのか、どの順で進めるか、の判断です。
...
**このリポジトリから、開発時に使っていた Supabase には接続できません。** 接続情報を含めていないためです。

- `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` / `JWT_SECRET` はリポジトリに一切含まれていません。`.env` は `.gitignore` で除外してあり、全コミットの履歴を通して一度もコミットされていません
- `.env.sample` に書かれているのはプレースホルダだけです
- `.env` が無い状態では `server/supabaseClient.js` がダミーの接続先（`http://localhost:54321`）にフォールバックします。開発サーバーは起動しますが、DB を参照する操作はすべて失敗します
- 動かして試す場合は、自分で Supabase のプロジェクトを用意し、`sql/schema.sql` を実行したうえで `.env` を設定してください（手順は上の「セットアップ」）

アプリケーションのコードには手を入れていません。接続情報が無いだけです。
...
- 認証情報はリポジトリに含まれていません。詳しくは上の「データベースへの接続について」を参照してください
